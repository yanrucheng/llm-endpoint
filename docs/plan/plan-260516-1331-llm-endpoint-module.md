...
**P4 Gate status:** Passed on 2026-05-16. Plain-text invocation, cancellation and late-response semantics, fake-provider scenario fixtures, role-health evaluation, redacted debug replay artifacts, and Zero BC public surface coverage are verified by Phase 4 contract tests.

| Check | Evidence |
|---|---|
| P4A plain text path | `llm_endpoint.router.route_invocation` returns `PlainTextResult` through the canonical invocation path without requiring schema resolution, while preserving deadline, policy, telemetry, and typed failure behavior. |
| P4B cancellation and late response | The router stops before attempts when already cancelled, emits cancellation and failure telemetry, discards cancelled or late provider successes, and does not fail over after cancellation or local timeout discard. |
...
| Zero BC policy | New Phase 4 public surfaces are direct `v1` clean-slate contracts; no version routers, deprecated status mapping, compatibility facades, or legacy fixture layouts were added. |
